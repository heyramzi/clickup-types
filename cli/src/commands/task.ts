...
export async function runTaskGetCommand(taskId: string, opts: { json?: boolean }): Promise<void> {
  ...
  printKeyValue([
    ...
    [
      "Start Date",
      task.start_date ? new Date(parseInt(task.start_date, 10)).toLocaleString() : null,
    ],
...
export async function runTaskCreateCommand(opts: {
  ...
  if (opts.description) {
    const key = opts.markdown ? "markdown_description" : "description";
    data[key] = opts.description;
  }
